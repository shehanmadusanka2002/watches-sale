...
const ProductGrid = ({ categoryType, showFilters = false }: ProductGridProps) => {
  ...
  const handleFilterChange = (filters: any) => {
    ...
    if (filters.movement) result = result.filter(p => p.movementType === filters.movement);
    ...
    <div className="w-full">
      {showFilters && (
        <div className="mb-16">
          <FilterBar 
            ...
            movements={Array.from(new Set(products.map(p => p.movementType).filter((m): m is string => Boolean(m))))} 
          />
        </div>
      )}
      ...
      {filteredProducts.length === 0 ? (
        <div className="py-24 text-center border border-white/5 bg-[#111111]">
           <p className="text-[10px] font-black uppercase tracking-[0.4em] text-zinc-500 italic">No timepieces matching your criteria</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-8 gap-y-24">
          {filteredProducts.map((product, idx) => (
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <div className="divider-line w-8 mb-4 group-hover:w-16 transition-all duration-700" />
